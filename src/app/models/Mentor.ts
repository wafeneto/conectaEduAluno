...
export class Mentor {
	...
	static UrlRequest: String = "http://66.147.225.75:8080/assistenciaSocial/";
	...
	static executaVisaoModulo(modulo, visao, parametros) {
		...
		return Mentor.tipefy(eval("(" + data + ")"));
	}
	...
	static strigToDateIso(
		dataString: String
	) {
		return new Date(Date.parse(dataString + ""));
	}

	static stringToDateIso(
		dataString: String
	) {
		return new Date(Date.parse(dataString + ""));
	}
	...
	static stringToDate(dataString: String) { //RECEBER STRING NO FORMATO DD/MM/YYYY
		try {
	...
	static dateToString(data: Date) { //RECEBER DATE
		try {
			if (data != null) {
	...
	static stringToMoeda(moedaString: String) {// RECEBER STRING VALOR NO FORMATO "2.000,00"
		try {
	...
	static rodaTransacaoFromObjeto(transacao, nomeCampo, objetos, trataRetorno: boolean) {
	...
	public decompressFromBase64(input) {
		...
		return this._decompress(input.length, 32, function (index) { return this.getBaseValue(keyStrBase64, input.charAt(index)); });
	}

	public compressToUTF16(input) {
		if (input == null) return "";
		return this._compress(input, 15, function (a) { if (f(a + 32) == "&") return "&"; return f(a + 32); }) + " ";
	}
	...
	public decompressFromEncodedURIComponent(input) {
		...
		return this._decompress(input.length, 32, function (index) { return this.getBaseValue(keyStrUriSafe, input.charAt(index)); });
	}
	...
	public static tipefy(obj: any){
		...
		if(Array.isArray(obj)){
			if(obj.length == 0)
				...
				classe = Mentor.capitalize(classe);
				var cls : any = null;
				cls = LibNamespace[classe];
				
				var lista = cls.criaColecao(obj);
				for(var x = 0;x<lista.length;x++)
					lista[x].mentorClasse = obj[0].mentorClasse;
				return lista;
			}
	
		}else{
			var classArr= obj.mentorClasse.split(".");
			var nome = obj.mentorClasse;
			var classe : string = classArr[classArr.length-1];
			classe = Mentor.capitalize(classe);
			var cls : any = null;
			cls = LibNamespace[classe];
			var retorno = new cls(obj);
			retorno.mentorClasse = obj.mentorClasse;
			return retorno;
